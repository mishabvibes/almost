...
export default function AutoSubscriptionsPage() {
  ...
  useEffect(() => {
    const fetchSubscriptions = async () => {
      ...
      setError(null);
      try {
        const response = await fetch("/api/subscriptions/auto");
        if (!response.ok) throw new Error("Failed to fetch auto subscriptions");
        ...
        console.error("Error loading auto subscriptions:", error);
  ...
  const exportToCSV = () => {
    const headers = ["ID", "Name", "Phone", "Amount", "Period", "Payment Status", "Last Payment", "Razorpay Subscription ID"];
    ...
    link.href = url;
    link.download = "auto_subscriptions.csv";
    link.click();
  };
